...
/**
 * Single-property website for the Baltimore residence (Olive Hollow LLC).
 * The component attempts to retrieve property records from Supabase and
 * filters to the Baltimore property only. If the database query fails,
 * it falls back to a single hard-coded Baltimore listing.
 */
  ...
  useEffect(() => {
    async function loadProperties() {
      try {
        ...
        if (!error && data) {
          // Filter to Baltimore only (avoid publicly tying other properties to this site).
          const filtered = (data as Property[]).filter((p) => {
            const hay = `${p.name} ${p.address} ${p.city} ${p.state} ${p.postal_code || ''}`.toLowerCase();
            return hay.includes('baltimore') || hay.includes('md') || hay.includes('4037') || hay.includes('edgewood');
          });
          setProperties(filtered.length ? filtered : []);
          return;
        }
      } catch (err) {
        // ignore; fall back to sample data
      }
      // Fallback sample data to illustrate the UI (Baltimore only)
      setProperties([
        {
          id: 1,
      ...
      <main>
          ...
          <div className="grid gap-6 rounded-3xl border border-white/10 bg-ink-800/70 p-8 shadow-luxe md:grid-cols-2 md:items-center">
            <div>
              <p className="inline-flex items-center rounded-full bg-white/5 px-3 py-1 text-xs font-medium text-champagne-200 ring-1 ring-white/10">
                4037 Edgewood Road Residences
              </p>
              <h1 className="mt-4 text-3xl font-semibold tracking-tight md:text-4xl">
                Boutique Baltimore living,
                <span className="text-white/60"> professionally managed</span>
              </h1>
              <p className="mt-3 max-w-prose text-white/70">
                Schedule a private tour in minutes and apply online securely. Residents can pay rent and
                submit maintenance requests through the portal.
              </p>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0b1a12] via-[#13281c] to-slate-900 p-8 text-white">
              ...
              <h2 className="relative text-lg font-semibold">Leasing in three steps</h2>
              <ol className="mt-4 space-y-3 text-sm text-white/90">
                <li className="flex gap-3">
                  ...
                  <span>Schedule a private tour (8am to 8pm Eastern Time).</span>
                </li>
                <li className="flex gap-3">
                  <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-white/10">2</span>
                  <span>Pay the $45 application fee and submit your application securely.</span>
                </li>
                <li className="flex gap-3">
                  <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-white/10">3</span>
                  <span>Receive a decision quickly and finalize your lease.</span>
                </li>
              </ol>
              <p className="mt-6 text-xs text-white/70">
                Questions? Email us anytime and our leasing team will respond promptly.
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="listings" className="mx-auto max-w-6xl px-4 pb-16">
          <div className="flex items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight">Now leasing</h2>
              <p className="mt-1 text-sm text-white/70">View details, schedule a tour, or apply online.</p>
            </div>
          </div>
