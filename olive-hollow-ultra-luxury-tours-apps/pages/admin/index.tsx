    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-slate-600">Applications, maintenance, and operations overview.</p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
              ...
              <ul className="mt-4 space-y-3">
                {applications.map((app) => (
                  <li key={app.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    <p className="mt-1 text-sm text-slate-600">Unit: {app.unit_id} · Status: {app.status}</p>
                  </li>
                ))}
              </ul>
            )}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
            ...
            {requests.length === 0 ? (
              <p className="mt-4 text-sm text-slate-600">No maintenance requests.</p>
            ) : (
              <ul className="mt-4 space-y-3">
                {requests.map((req) => (
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
          <h3 className="text-base font-semibold">Next improvements (optional)</h3>
